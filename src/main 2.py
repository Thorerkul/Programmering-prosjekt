...
def startGame(map):
    ...
    if map == "boss":
        ...
        bgimg = pg.image.load(r'src\assets\art\maps\pixil-frame-0 (4).png').convert_alpha()
        bgimg = pg.transform.scale(bgimg, (SCREEN_WIDTH, SCREEN_HEIGHT))
        bgrect = pg.Rect(0, 0, window.current_w, window.current_h)
...
def loadSaves(dir='saves'):
    ...
    dir = 'src\\assets\saves\\level1.txt'
    levela = open(dir, 'a')
    levelr = open(dir, 'r')
...
class Player:
    ...
    def loadSprites(self):
        ...
        if self.char == "billy" or self.char == "dummy":
            ...
            self.frozenSprite = pg.image.load(r'src\assets\art\karakterer\Billy\billyFrozen.png').convert_alpha()
            self.stuckSprite = pg.image.load(r'src\assets\art\karakterer\Billy\billyStuck.png').convert_alpha()
...
class MainMenu:
    def __init__(self):
        ...
        self.bgSurf = pg.image.load(r'src\assets\art\bg\mainBG.png')
...
def load_music(place):
    ...
    if place == "menu":
        y = r'src\assets\lyd\Music\A-Better-World.wav'
    if place == "credits":
        y = r'src\assets\lyd\Music\Lines-of-Code.wav'
